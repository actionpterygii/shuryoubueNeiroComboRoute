  ...
  <main class="m-2">
    ...
    <div class="block">
      <div class="notification is-primary is-light">
        <div class="content">
          <ul>
            <li>響周波の属性とかは旋律が同じでも武器によって異なっていてこのアプリケーションの構成上ややこいので省略しています</li>
            <li>「L」は左スティックのことです</li>
            <li>コマンドで(1)などの数字は音色です同じ武器動作でも音色が違うので分けています</li>
            <li>コマンドで(c)がついているのは特定のコンボのみで使えるコマンドです（例：通常はスティック入れないと出ない前方攻撃ですが左ぶん回しの後だとスティックなし△で出ます）</li>
            <li>コマンドの「休み」はコンボで連続して出せないコマンド（例：前方攻撃→前方攻撃）が出したいときのためです</li>
            <li>「⇒」のあるコマンドはひとつの攻撃動作の中に2回ボタンを押します（例：通常は前方攻撃になる抜刀攻撃に△+◯を追加で入力することで後方攻撃が出る）</li>
            <li>めんどくなるだけなのでここに入れてはいませんが、騎乗状態から好きな音色をストックするには△⇒◯or△+◯でできます</li>
          </ul>
        </div>
      </div>
    </div>
